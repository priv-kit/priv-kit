...
#define LOG_TAG "PrivilegeAdbPairing"
#include "privilege_adb_logging.h"
...
static jlong PairingContext_Constructor(JNIEnv *env, jclass, jboolean isClient, jbyteArray jPassword) {
    ...
    auto ctx = static_cast<PairingContextNative *>(malloc(sizeof(PairingContextNative)));
...
static jbyteArray PairingContext_Msg(JNIEnv *env, jobject, jlong ptr) {
    auto ctx = reinterpret_cast<PairingContextNative *>(ptr);
    jbyteArray our_msg = env->NewByteArray(ctx->key_size);
    env->SetByteArrayRegion(our_msg, 0, ctx->key_size, reinterpret_cast<jbyte *>(ctx->key));
    return our_msg;
}

static jboolean PairingContext_InitCipher(JNIEnv *env, jobject, jlong ptr, jbyteArray jTheirMsg) {
    auto ctx = reinterpret_cast<PairingContextNative *>(ptr);
    auto their_msg_size = env->GetArrayLength(jTheirMsg);
    if (their_msg_size > SPAKE2_MAX_MSG_SIZE) {
        LOGE("their_msg size [%d] greater than max size [%d].", their_msg_size, SPAKE2_MAX_MSG_SIZE);
        return JNI_FALSE;
    }
...
static jbyteArray PairingContext_Encrypt(JNIEnv *env, jobject, jlong ptr, jbyteArray jIn) {
    auto ctx = reinterpret_cast<PairingContextNative *>(ptr);
    auto aes_ctx = ctx->aes_ctx;
    auto in = env->GetByteArrayElements(jIn, nullptr);
    auto in_size = env->GetArrayLength(jIn);

    auto out_size = static_cast<size_t>(in_size) + EVP_AEAD_max_overhead(EVP_AEAD_CTX_aead(ctx->aes_ctx));
    ...
    if (!status) {
        LOGE("Failed to encrypt (in_len=%d, out_len=%" PRIuPTR")", in_size, out_size);
        return nullptr;
    }
    ++ctx->enc_sequence;

    jbyteArray jOut = env->NewByteArray(written_sz);
    env->SetByteArrayRegion(jOut, 0, written_sz, reinterpret_cast<jbyte *>(out));
    return jOut;
}

static jbyteArray PairingContext_Decrypt(JNIEnv *env, jobject, jlong ptr, jbyteArray jIn) {
    auto ctx = reinterpret_cast<PairingContextNative *>(ptr);
    auto aes_ctx = ctx->aes_ctx;
    auto in = env->GetByteArrayElements(jIn, nullptr);
    auto in_size = env->GetArrayLength(jIn);
    ...
    if (!status) {
        LOGE("Failed to decrypt (in_len=%d, out_len=%" PRIuPTR")", in_size, out_size);
        return nullptr;
    }
    ++ctx->dec_sequence;
...
static void PairingContext_Destroy(JNIEnv *, jobject, jlong ptr) {
    auto ctx = reinterpret_cast<PairingContextNative *>(ptr);
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *vm, void *) {
    ...
    auto clazz = env->FindClass("priv/kit/adb/PrivilegeAdbPairingContext");
    if (clazz == nullptr) {
        return -1;
    }
