...
The built-in static-TCP control dialog dismisses before dispatching its Stop or Restart action. Each action first connects through the active static endpoint. If that connection fails on Android 11 or later, the same action retries through the Wireless Debugging connect endpoint using the configured managed-Wireless-Debugging policy. Stop sends `usb:` through whichever connection succeeds; Restart sends `tcpip:<static port>` and keeps the configured static port unchanged.
