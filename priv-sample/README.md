...
- Authorization tests are split into independent Root, manual shell, Shizuku-backed external start, Wireless ADB, TCP mode, and session-log tabs.
...
- A sample Shizuku UserService starter that runs the shared detached Priv Kit external start command, then immediately destroys the Shizuku UserService while the app waits for the privileged server handoff.

The sample demonstrates the Root runtime minimum loop, manual shell verification path, Shizuku-backed external start path, Wireless ADB startup path, one app-owned hidden API smoke test for Binder remote transact, and two app-owned UserService AIDL smoke tests. Both sample UserServices accept `Context`; the embedded UserService omits `destroy()`, while the dedicated UserService exits from `destroy()` with `System.exit(0)`. It does not demonstrate reusable Android system service wrappers.
