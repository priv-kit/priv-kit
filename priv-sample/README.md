...
- UserService tests for binding and calling separate app-owned AIDL services in the default dedicated-process mode and the explicit embedded-in-server mode.
...
The sample demonstrates the Root runtime minimum loop, manual shell verification path, Wireless ADB startup path, one app-owned hidden API smoke test for Binder remote transact, and two app-owned UserService AIDL smoke tests. The embedded UserService omits `destroy()`, while the dedicated UserService exits from `destroy()` with `System.exit(0)`. It does not demonstrate Delegate or reusable Android system service wrappers.
