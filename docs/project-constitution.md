...
Root 和 ADB 策略必须把自己的传输细节和失败细节留在 `:priv-runtime` 内部。手动命令和外部启动命令由运行时生成，并通过同一条 Binder handoff 完成连接。可在外部特权进程内执行启动命令的通用 helper 和主进程日志接收 helper 属于 `:priv-runtime`；Shizuku 等第三方绑定代码只能留在应用侧、可选集成或 sample。通用 native starter 可执行文件属于 `:priv-runtime`。
...
- `:priv-runtime` 负责客户端编排、Root 启动、ADB 启动、手动命令、外部启动命令、外部特权进程执行 helper、主进程日志接收 helper、通用 native starter、Privileged Server 进程行为、Binder/UserService 原语、内部 AIDL、wire contract、handshake registry、服务端侧 UserService 生命周期和独立 UserService 进程入口。
