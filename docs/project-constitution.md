...
- `:priv-core` 共享 `priv.kit.core`、`priv.kit.binder`、`priv.kit.userservice`
- `:priv-runtime` 使用 `priv.kit.runtime`
- `:priv-server` 使用 `priv.kit.server`，并承载服务端侧 `priv.kit.userservice` 实现
- `:priv-adb-crypto` 使用 `priv.kit.adb.crypto.certificate` 和 `priv.kit.adb.crypto.pairing`
- `:priv-adb` 使用 `priv.kit.adb`
- `:priv-ui` 使用 `priv.kit.ui`

保留 `priv.kit.binder` 和 `priv.kit.userservice` package 分区是为了维持协议语义清晰；它们不要求对应独立 Gradle 模块。
...
Root 策略必须把自己的传输细节和失败细节留在 `:priv-runtime` 内部；ADB 策略必须把自己的传输细节和失败细节留在 `:priv-adb` 内部。手动命令和外部启动命令由运行时生成，并通过同一条 Binder handoff 完成连接。可在外部特权进程内执行启动命令的通用 helper 和主进程日志接收 helper 属于 `:priv-runtime`；Shizuku 等第三方绑定代码只能留在应用侧、可选集成或 sample。通用 native starter 可执行文件属于 `:priv-runtime`，不得放入 ADB 专用模块。
...
- `:priv-runtime` 负责客户端编排、Root 启动执行、手动命令、外部启动命令、外部特权进程执行 helper、主进程日志接收 helper 和通用 native starter 可执行文件。
- `:priv-server` 负责 Privileged Server 进程行为。
- `:priv-core` 同时承载 Binder/UserService 的 AIDL、共享模型、共享异常、wire contract、handshake registry 和 raw Binder primitive。
- `:priv-server` 承载服务端侧 UserService registry、manager、host、loader、process binder、destroyer 和独立 UserService 进程入口。
...
- `:priv-adb` 负责 ADB 启动的 pairing/connect、命令执行通道和失败建模。
...
示例可以包含最小 hidden API 调用来验证底层 Binder remote transact 是否可用，但该调用必须停留在 `:priv-sample`，不得上升为 `:priv-runtime`、`:priv-core` 或其他发布模块中的类型化 Android 系统服务 API。发布模块只能暴露显式服务名的 raw Binder transaction 桥，不得把示例里的系统服务接口封装成项目 API。
