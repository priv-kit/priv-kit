...
- `:priv-bc` 使用 `priv.kit.bc`
- `:priv-ssl` 使用 `priv.kit.ssl`
...
Root 策略必须把自己的传输细节和失败细节留在 `:priv-runtime` 内部；ADB 策略必须把自己的传输细节和失败细节留在 `:priv-adb` 内部。手动命令和外部启动命令由运行时生成，并通过同一条 Binder handoff 完成连接。
...
- `:priv-runtime` 负责客户端编排、Root 启动执行、手动命令和外部启动命令。
