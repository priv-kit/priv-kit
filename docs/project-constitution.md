...
Shizuku、Dhizuku 或类似授权工具只能作为外部启动命令的执行入口存在。它们不能扩展成通用特权服务市场或全局权限代理。
...
Root 策略必须把自己的传输细节和失败细节留在 `:priv-runtime` 内部；ADB 策略必须把自己的传输细节和失败细节留在 `:priv-adb` 内部。手动命令和外部启动命令由运行时生成，并通过同一条 Binder handoff 完成连接。通用 native starter 可执行文件属于 `:priv-runtime`，不得放入 ADB 专用模块。
...
- `:priv-runtime` 负责客户端编排、Root 启动执行、手动命令、外部启动命令和通用 native starter 可执行文件。
