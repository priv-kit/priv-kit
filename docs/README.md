...
当前 Binder 阶段提供显式目标 Binder 的 remote transact 原语、显式系统服务名的 raw Binder transact 桥和统一 server 不可用失败语义。公开 Binder 原语位于 `:priv-core` 的 `priv.kit.core.binder` package 分区；内部 AIDL 和 server transaction 实现位于 `priv.kit.core.internal.*`。项目自有 server control 调用在 server Binder 缺失或死亡时暴露为 `PrivilegeServerUnavailableException`；宿主可以用 `PrivilegeBinderCall.orElse(...)` 为 server 不可用或直接 Binder endpoint 死亡提供显式 fallback。UserService、其他 `RemoteException` 和目标 Binder 调用失败按 raw Binder 语义透传给调用方处理，不推测死亡来源。它不提供 Binder endpoint 注册槽位，不提供 Android 系统服务类型化代理，也不扩展成高级系统能力封装。
