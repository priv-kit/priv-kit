...
项目提供特权执行基础设施，不提供通用特权 Android 操作库，也不提供 AndroidX 风格的系统 API 兼容层。内置文件代理是一个受限例外：它只代理绝对路径上的基础文件操作和目录遍历，不扩展为存储管理策略或 Android 系统服务 facade。
...
目录遍历必须通过 `PrivilegeFile.walk(maxDepth)` 暴露为无排序、深度优先先序、弱一致的冷流，并使用有界并发的服务端协程和 `ParcelFileDescriptor` pipe 流式返回，不得把完整目录或目录树装入一个 Binder 返回值。接收者目录本身不输出，直接子项深度为 1，`maxDepth = 1` 表示非递归列出直接子项，默认深度为 `Int.MAX_VALUE`。服务端先枚举目录项名称，再以 `lstat` 尽力读取元数据；因 `EACCES` 或 `EPERM` 无法读取属性时必须保留名称、返回空元数据且不得进入该条目，条目在读取属性前消失时跳过。符号链接必须输出但不得进入；进入后代目录必须使用 `SecureDirectoryStream` 的描述符相对操作并禁止跟随符号链接。起始目录或任一待进入目录无法打开、迭代或发生其他不可恢复错误时终止整个 walk 并保留 errno。调用方取消、owner 死亡和服务端关闭必须取消协程并关闭所有目录流和 pipe。一次 walk 无论深度只占一个服务端槽位；服务端同时最多执行四个 walk 且不排队，超出容量返回 `EBUSY`。

文件代理允许通过挂起的 `deleteRecursively()` 删除一个显式绝对路径及其后代。客户端公开方法必须先按词法合并重复分隔符及 `.`、`..` 段，并拒绝合并后指向文件系统根的目标，再把合并结果传给服务端。Binder 线程只能完成无排队并发准入、任务登记和协程调度，不得重复校验或改写目标路径；遍历必须在服务端有界 IO 协程中执行，同时最多四个。取消调用方协程、owner 死亡和服务端关闭必须取消相应任务，但已经删除的条目不回滚。递归删除不得跟随符号链接；目录遍历必须使用 `SecureDirectoryStream` 的描述符相对操作，不支持安全目录流时不得退化为存在符号链接竞态的路径遍历。返回值保持 `File.deleteRecursively()` 的布尔语义：目标不存在或全部删除时为 `true`，任一条目无法删除时为 `false`，并允许继续尽力删除其余条目。

文件代理仍不得提供存储清理、路径发现、授权策略、跨应用共享守护进程或可配置的破坏性遍历策略。
